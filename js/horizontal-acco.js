...
const closeItem = (item) => {
  ...
  itemParent.classList.remove("acco__item--active");
  // снимаем у кнопки активный класс
  item.classList.remove("acco__button--active");
...
const openItem = (item) => {
  ...
  itemParent.classList.add("acco__item--active");
  // добавляем самой кнопке активный класс
  item.classList.add("acco__button--active");
...
menu.addEventListener("click", (e) => {
  ...
  const isActive = target.classList.contains("acco__button--active");
  // находим есть ли вообще на странице активный класс
  const activeElement = document.querySelector(".acco__button--active");

  // если мы кликнули по кнопке уже открытого элемента
  if (target.classList.contains("acco__button") && isActive) {
...
window.addEventListener("resize", () => {
  ...
  const activeButton = document.querySelector(".acco__button--active");
  // если она есть
  if (activeButton) {
